...
(function($){
	function crateCommentInfo(obj){
		...
		var el = "<div class='comment-info'><div class='comment-content-header'>作者：<span class='auth'>"+obj.replyName+"</span><span><i class='glyphicon glyphicon-time'></i>"+obj.time+"</span></div><div class='comment-right'>"			
	...
	//			console.log(el)//回复
				var obj = new Object();
				obj.replyName=author;
				if(el.parent().parent().hasClass("reply")){
					obj.beReplyName = el.parent().parent().find("a:first").text();
				}else{
					obj.beReplyName=parentEl.siblings('.comment-content-header').find(".auth").text();
				}
				obj.content=content;
				obj.time = getNowDateFormat();
				var replyString = createReplyComment(obj);
				if(obj.replyName==obj.beReplyName){
					var replyvall = "<div class='reply'><div><a href='javascript:void(0)' class='replyname'>"+obj.replyName+"</a>:<span>"+obj.content+"</span></div>"
						+ "<p><span>"+obj.time+"</span> <span class='reply-list-btn'>回复</span></p></div>";
						parentEl.find(".reply-list").append(replyvall).find(".reply-list-btn:last").click(function(){alert("不能回复自己");});
				}else{
...
//				parentEl.find(".reply-list").append(replyString).find(".reply-list-btn:last").click(function(){alert("不能回复自己");});
				var replyval = "<div class='replys'><div><a href='javascript:void(0)' class='replyname'>"+obj.replyName+"</a>:<a href='javascript:void(0)'>@"+obj.beReplyName+"</a><span>"+obj.content+"</span></div>"
						+ "<p><span>"+obj.time+"</span> <span class='reply-list-btn'>回复</span></p></div>";
				$(this).parent().parent(".reply").append(replyval).find(".reply-list-btn:last").click(function(){alert("不能回复自己")});;
				$(".replybox").remove();
			}else{
				alert("空内容");
			}
...
//				replyClick($(this));
//			});
			$(this).prepend(str).find(".reply-btn").unbind().click(function(){
				if($(this).parent().parent().find(".replybox").length > 0){
					$(".replybox").remove();
				}else{
					$(".replybox").remove();
